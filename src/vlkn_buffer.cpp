 ...
 */
VkDeviceSize VlknBuffer::getAlignment(VkDeviceSize instanceSize,
...
VlknBuffer::VlknBuffer(VlknDevice &device, VkDeviceSize instanceSize,
                       uint32_t instanceCount, VkBufferUsageFlags usageFlags,
                       VkMemoryPropertyFlags memoryPropertyFlags,
                       VkDeviceSize minOffsetAlignment)
    : vlknDevice{device}, instanceSize{instanceSize},
...
VlknBuffer::~VlknBuffer() {
  unmap();
  vkDestroyBuffer(vlknDevice.device(), buffer, nullptr);
  vkFreeMemory(vlknDevice.device(), memory, nullptr);
}
 ...
 * @return VkResult of the buffer mapping call
 */
VkResult VlknBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
  assert(buffer && memory && "Called map on buffer before create");
  return vkMapMemory(vlknDevice.device(), memory, offset, size, 0, &mapped);
}
 ...
 */
void VlknBuffer::unmap() {
  if (mapped) {
    vkUnmapMemory(vlknDevice.device(), memory);
    mapped = nullptr;
  }
}
 ...
 *
 */
void VlknBuffer::writeToBuffer(const void *data, VkDeviceSize size,
                               VkDeviceSize offset) {
 ...
 */
VkResult VlknBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
  VkMappedMemoryRange mappedRange = {};
  mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
  mappedRange.memory = memory;
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkFlushMappedMemoryRanges(vlknDevice.device(), 1, &mappedRange);
}
 ...
 */
VkResult VlknBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
  VkMappedMemoryRange mappedRange = {};
  mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
  mappedRange.memory = memory;
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkInvalidateMappedMemoryRanges(vlknDevice.device(), 1, &mappedRange);
}
 ...
 */
VkDescriptorBufferInfo VlknBuffer::descriptorInfo(VkDeviceSize size,
                                                  VkDeviceSize offset) {
 ...
 *
 */
void VlknBuffer::writeToIndex(void *data, int index) {
 ...
 *
 */
VkResult VlknBuffer::flushIndex(int index) {
 ...
 */
VkDescriptorBufferInfo VlknBuffer::descriptorInfoForIndex(int index) {
 ...
 * @return VkResult of the invalidate call
 */
VkResult VlknBuffer::invalidateIndex(int index) {
